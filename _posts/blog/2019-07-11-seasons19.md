...
The growth system in Seasons 19 has received a revamp. No longer is every year the same and neither is success predetermined. Look out for bad weather destroying your crops, whether it be frost or draughts. Also plan a year ahead to keep your fields nutricious using crop rotation.
...
Crop rotation is the practice to grow different crops in succession that are beneficial to each other. This is now possible in Seasons and with successful planning and execution there will be increased yields and reduced need to fertilization. For the most beneficial rotations and crops a yield effect of 120% of vanilla will be possible.


Crops are divided into crop categories and have dependencies that determines whether a preceding crop category is favourable, neutral or unfavourable for yield. Some crops are more suitable to grow several years in succession while others who require more variation.


It is also possible to leave a field to rest and restore for a year (in the mod termed keeping it fallow). Most common crop rotations should be possible to simulate, including the effects of double cropping.
...
Seasons 19 includes real life animal breeds and each breed has its purpose. Some breeds are meant for meat production, while there are specific breeds for milk, wool and egg production.
